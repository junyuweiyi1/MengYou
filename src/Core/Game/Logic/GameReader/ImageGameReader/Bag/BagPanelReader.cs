...
public sealed class BagPanelReader
{
    ...
    public BagPanelReader(IVisionServiceMgr vision, IUIElementLocateMgr locator)
    {
        _vision = vision;
        _locator = locator;
    }
    ...
    private bool HasItem(Rect region)
    {
        ...
        for (var dy = -1; dy <= 1; dy++)
        {
            ...
            {
                var argb = _vision.ReadPixel(new Vector2(cx + dx, cy + dy));
    ...
    private string ReadItemName(string slotKey, Rect iconRegion)
    {
        ...
        //{
        //    if (!_vision.HasTemplate($"ItemIcon.{candidate}")) continue;
        //    var hit = _vision.FindImage($"ItemIcon.{candidate}", MatchThreshold);
